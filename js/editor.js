	...
	var methods = {
		...
		imageWidget: function(){
			...
			var getURL = $("<button/>",{
				...
				if(url ==''){
					methods.showMessage.apply(this,["imgErrMsg","请输入图像地址"]);
					return false;
				}
				...
				class:"row-fluid"
					...
					id:"tblForm"+idExtn 
						...
						class:"row" 
							...
							).append($('<label/>',{	for:"tblBorder"+idExtn, text:"边框大小"}
							)).append($('<input/>',{ 
								id:"tblBorder"+idExtn,
								type:"text",
								class:"form-control form-control-width",
						   ...
						   'insert_img'	: { "modal": true,
											...
											"onSave": function(){
												...
												if($('#imageList').data('current')){
													...
													methods.showMessage.apply(this,["imgErrMsg","请选择图像文件"]);
													return false;
												}
			...
			var statusBar = $("<div/>", {	id : "statusbar",
	       	...
	       	if(settings['status_bar']){
				editor.keyup(function(event){
					...
					$(editor_Content).data("statusBar").append('<div class="label">'+'已输入文字 : '+charCount+'</div>');
            	});
	        }	        
